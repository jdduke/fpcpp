...
template<size_t FArgC> struct curry_helper_impl { };
  ...
  template<> struct curry_helper_impl<1> {
    ...
    static inline auto bind(F f, T t) RETURNS( FP_CURRY1(P_PREFIX) );
   };
  template<> struct curry_helper_impl<2> {
    template<typename F,typename T>
    static inline auto bind(F f, T t) RETURNS( FP_CURRY2(FP_PREFIX1) );
    template<typename F, typename T, typename T1>
    static inline auto bind2(F f, T t, T1 t1) RETURNS( FP_CURRY1(FP_PREFIX2) );
  };
  template<> struct curry_helper_impl<3> {
    template<typename F,typename T>
    static inline auto bind(F f, T t) RETURNS( FP_CURRY3(FP_PREFIX1) );
    template<typename F, typename T, typename T1>
    static inline auto bind2(F f, T t, T1 t1) RETURNS( FP_CURRY2(FP_PREFIX2) );
    template<typename F, typename T, typename T1, typename T2>
    static inline auto bind3(F f, T t, T1 t1, T2 t2) RETURNS( FP_CURRY1(FP_PREFIX3) );
  };
  template<> struct curry_helper_impl<4> {
    template<typename F,typename T>
    static inline auto bind(F f, T t) RETURNS( FP_CURRY4(FP_PREFIX1) );
    template<typename F, typename T, typename T1>
    static inline auto bind2(F f, T t, T1 t1) RETURNS( FP_CURRY3(FP_PREFIX2) );
    template<typename F, typename T, typename T1, typename T2>
    static inline auto bind3(F f, T t, T1 t1, T2 t2) RETURNS( FP_CURRY2(FP_PREFIX3) );
    template<typename F, typename T, typename T1, typename T2, typename T3>
    static inline auto bind4(F f, T t, T1 t1, T2 t2, T3 t3) RETURNS( FP_CURRY1(FP_PREFIX4) );
  };
#else
