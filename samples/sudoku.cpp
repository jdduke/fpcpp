...
static const let subRows = cons(show("ABC"),  cons(show("DEF"), cons(show("GHI"), strings())));
static const let subCols = cons(show("123"), cons(show("456"), cons(show("789"), strings())));
static const let unitlist = concat( concat( mapCrossInner(cols, rows),
                                            mapCrossInner(rows, cols) ),
                                    mapCross( subRows, subCols ) );
